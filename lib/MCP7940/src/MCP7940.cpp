...
bool MCP7940_Class::begin(const int sdaPin, const int sclPin, const uint32_t i2cSpeed) const
{
  /*!
...
bool MCP7940_Class::deviceStop() const
{
  ...
  { // Loop until changed or overflow
    oscillatorStatus =
...
int8_t MCP7940_Class::calibrateOrAdjust(const DateTime &dt)
{
  ...
  if ((ppm > 130) ||
      (ppm < -130))
  { // calibration is out of range so just set the time  DML 2/5/2019
    adjust(dt);
