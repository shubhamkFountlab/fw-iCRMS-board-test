...
ModbusClientRTU::ModbusClientRTU(int8_t rtsPin, uint16_t queueLimit) : ModbusClient(),
                                                                       MR_serial(nullptr),
...
ModbusClientRTU::ModbusClientRTU(RTScallback rts, uint16_t queueLimit) : ModbusClient(),
                                                                         MR_serial(nullptr),
                                                                         MR_lastMicros(micros()),
                                                                         MR_interval(2000),
                                                                         MTRSrts(rts),
...
void ModbusClientRTU::sendDataRs485Serial(uint8_t *data, uint8_t len)
{
  // Clear serial buffers
  while (MR_serial->available())
    MR_serial->read();
  // RTU mode
  uint16_t crc16 = RTUutils::calcCRC(data, len);

  // Respect interval - we must not toggle rtsPin before
  if (micros() - MR_lastMicros < MR_interval)
    delayMicroseconds(MR_interval - (micros() - MR_lastMicros));

  // Toggle rtsPin, if necessary
  MTRSrts(HIGH);
  // Write message
  MR_serial->write(data, len);
  // Write CRC in LSB order
  MR_serial->write(crc16 & 0xff);
  MR_serial->write((crc16 >> 8) & 0xFF);
  MR_serial->flush();
  // Toggle rtsPin, if necessary
  MTRSrts(LOW);
}

uint32_t ModbusClientRTU::serialAvailable()
{
  return MR_serial->available();
}

uint32_t ModbusClientRTU::serialRead()
{
  return MR_serial->read();
}
